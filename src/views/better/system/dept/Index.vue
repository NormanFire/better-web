...
export default {
  ...
  methods: {
    ...
    initDeptTree() {
      this.$get('upms/dept').then((r) => {
        this.deptTree = r.data.data.rows
      })
    },
    exportExcel() {
      this.$download('upms/dept/excel', {
    ...
    deleteDept() {
      ...
      if (checked.length === 0) {
        ...
        this.$confirm(this.$t('tips.confirmDeleteNode'), this.$t('common.tips'), {
          ...
          this.$delete(`upms/dept/${this.dept.deptIds}`).then(() => {
            this.$message({
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.dept.deptId) {
            this.$put('upms/dept', { ...this.dept }).then(() => {
            ...
            this.$post('upms/dept', { ...this.dept }).then(() => {
              this.buttonLoading = false
              this.$message({
