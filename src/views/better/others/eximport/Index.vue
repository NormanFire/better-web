...
export default {
  ...
  data() {
    return {
      uploadAction: `${process.env.VUE_APP_BASE_API}upms/eximport/import`,
      dialogVisible: false,
      tableKey: 0,
      loading: false,
      list: null,
  ...
  methods: {
    ...
    templateDownload() {
      this.$download('upms/eximport/template', {}, 'excel_import_template.xlsx')
    },
    exportExcel() {
      this.$download('upms/eximport/excel', {
    ...
    fetch(params = {}) {
      ...
      this.$get('upms/eximport', { ...params }).then((r) => {
